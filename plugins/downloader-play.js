...
`.trim();
    ...
    if (command == 'play') {
    try {      
        const info = await youtubedl(yt_play[0].url, { dumpJson: true, noCheckCertificates: true, noWarnings: true, preferFreeFormats: true });
        const audioFormats = info.formats.filter(f => f.acodec !== 'none' && f.vcodec === 'none');
        const dl_url = audioFormats.length > 0 ? audioFormats[0].url : info.url;
        await conn.sendMessage(m.chat, {audio: {url: dl_url}, fileName: yt_play[0].title + '.mp3', mimetype: 'audio/mpeg'}, {quoted: m});
    } catch (e) {
        console.log(e);
        throw '> ⓘ 𝙊𝙘𝙪𝙧𝙧𝙞𝙤 𝙪𝙣 𝙚𝙧𝙧𝙤𝙧 𝙚𝙣 𝙡𝙖 𝙙𝙚𝙨𝙘𝙖𝙧𝙜𝙖.';
    }
}
    if (command == 'youtube') {
    try {
        const info = await youtubedl(yt_play[0].url, { dumpJson: true, noCheckCertificates: true, noWarnings: true, preferFreeFormats: true });
        const videoFormats = info.formats.filter(f => f.acodec !== 'none' && f.vcodec !== 'none' && f.ext === 'mp4');
        const dl_url = videoFormats.length > 0 ? videoFormats[videoFormats.length - 1].url : info.url;
        await conn.sendMessage(m.chat, {video: {url: dl_url}, fileName: yt_play[0].title + '.mp4', mimetype: 'video/mp4'}, {quoted: m});
    } catch (e) {
        console.log(e);
        throw '> ⓘ 𝙊𝙘𝙪𝙧𝙧𝙞𝙤 𝙪𝙣 𝙚𝙧𝙧𝙤𝙧 𝙚𝙣 𝙡𝙖 𝙙𝙚𝙨𝙘𝙖𝙧𝙜𝙖.';
    }
